...
| `visualization.py` | Matplotlib figure writers (`write_demo_convergence_figure`, `write_convergence_tick_histogram`), also moved out of `scripts/`. They return `None` only for honestly unplottable data; `run_publication_analysis` in [`analysis.py`](../src/template_formal/colony/analysis.py) raises on either missing required figure, so stale or absent publication visuals cannot pass the analysis stage. |
...
- **`scripts/`** contains three thin orchestrators:
  ...
  convention reuse),
  [`02_run_analysis.py`](../scripts/02_run_analysis.py), which resolves the
  project root, delegates to `colony.analysis.run_publication_analysis` — the
  source-owned service that writes the JSON summaries, figures, and
  `figure_registry.json` — and prints the returned output paths for manifest
  collection, and
