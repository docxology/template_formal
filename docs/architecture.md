...
| `visualization.py` | Matplotlib figure writers (`write_demo_convergence_figure`, `write_convergence_tick_histogram`), also moved out of `scripts/`. They return `None` only for honestly unplottable data; `scripts/pipeline/stage_02_analysis.py` treats either missing required figure as a hard failure, so stale or absent publication visuals cannot pass the analysis stage. |
...
- **`scripts/`** contains three thin orchestrators:
  [`stage_00_setup.py`](../../../../scripts/pipeline/stage_00_setup.py) (repo
  convention reuse),
  [`stage_02_analysis.py`](../../../../scripts/pipeline/stage_02_analysis.py), which wires real
