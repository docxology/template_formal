...
| `05_results_discussion.md` | Results and Discussion (`{#sec:results-discussion}`) | The evidentiary core — mypy-as-oracle proof-of-detection, fault-injected negative controls, colony convergence statistics, eleven pre-registered analyses across three experiment families, the formal side-spec status, and threats to the claims. By far the largest file; read its own headings before editing (`grep -n '^##' 05_results_discussion.md`). |
