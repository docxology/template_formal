...
3. **Figure additions.** Only two figures exist
   ...
   written to `../output/figures/` during `scripts/02_run_analysis.py`. A new
   figure needs: a generator in `src/template_formal/colony/visualization.py`,
   a call site in `src/template_formal/colony/analysis.py`
   (`run_publication_analysis`, which is what the script invokes), a Pandoc image line with a
